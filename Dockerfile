# Copyright 2022 Google LLC
#
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
# You may obtain a copy of the License at
#
#      http://www.apache.org/licenses/LICENSE-2.0
#
# Unless required by applicable law or agreed to in writing, software
# distributed under the License is distributed on an "AS IS" BASIS,
# WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
# See the License for the specific language governing permissions and
# limitations under the License.


# syntax=docker/dockerfile:1

############################# BUILD ###############################
#This stage builds the HarbourBridge binary using its source code.

# Add golang as the base image for the build stage.
FROM golang:1.18.3 AS build

# Set the default work directory as “/harbourbridge_bin” in the container.
WORKDIR /harbourbridge_bin

# Copy HarbourBridge’s source code into the working directory.
COPY . ./
# Download HarbourBridge’s dependencies mentioned in the go.mod file.
RUN go mod tidy 
# Compile the source code and store the binary in the working directory /harbourbridge_bin.
RUN make build-static  

########################### RELEASE ###############################
#This stage consists of the GCloud SDK and the HarbourBridge binary.

# Add the Ubuntu base image from the Google Container Registry.
FROM gcr.io/cloud-marketplace/google/ubuntu1804@sha256:d9fe979ab72ff02f5b5d95c59cda6681aa73039fbf9b46596935267cf26c692e

# Create /harbourbridge directory to store its artifacts.
RUN mkdir -p /harbourbridge
# Create directory to store the dump files. This directory binds with the host directory that contains the dump files of the source database.
RUN mkdir -p /harbourbridge/source_dump
# Create directory to store the output files. This directory binds with the host directory that is to store the output files generated by HarbourBridge.
RUN mkdir -p /harbourbridge/harbour_bridge_output

# Copy the HarbourBridge binary from the build stage's harbourbridge_bin directory to the release stage's harbourBridge/bin directory.
COPY --from=build /harbourbridge_bin/harbourbridge /harbourbridge
# Copy the ui and webv2 directories into the release stage.
COPY --from=build /harbourbridge_bin/ui/. /harbourbridge/ui
COPY --from=build /harbourbridge_bin/webv2/. /harbourbridge/webv2

# Add Python3 to support GCloud SDK.
RUN apt-get update
RUN apt-get install -y python3.10

# Add NodeJS and Angular CLI to support the web UI.
WORKDIR /harbourbridge/ui
ENV VERSION=v16.16.0
ENV DISTRO=linux-x64
RUN curl https://nodejs.org/dist/v16.16.0/node-v16.16.0-linux-x64.tar.xz > /tmp/node-v16.16.0-linux-x64.tar.xz \
  && apt-get install xz-utils \
  && mkdir -p /usr/local/lib/nodejs \
  && tar -C /usr/local/lib/nodejs -xJf /tmp/node-v16.16.0-linux-x64.tar.xz 
ENV PATH /usr/local/lib/nodejs/node-v16.16.0-linux-x64/bin:$PATH
RUN npm install --location=global npm@8.14.0
RUN npm install --location=global @angular/cli@14.0.5

# Set the default working directory. 
WORKDIR /harbourbridge
# Shell script to run the webv2.
RUN echo "#!/bin/sh \ncd ui && ng serve & ./harbourbridge --webv2" >> webv2.sh 

# Download the gcloud package.
RUN curl https://dl.google.com/dl/cloudsdk/channels/rapid/downloads/google-cloud-cli-391.0.0-linux-x86_64.tar.gz > /tmp/google-cloud-sdk.tar.gz

# Install the gCloud package.
RUN mkdir -p /usr/local/gcloud \
  && tar -C /usr/local/gcloud -xf /tmp/google-cloud-sdk.tar.gz \
  && /usr/local/gcloud/google-cloud-sdk/install.sh

# Add gcloud to $PATH.
ENV PATH $PATH:/usr/local/gcloud/google-cloud-sdk/bin
